...
このサンプルを実行する詳しい手順は、[README.md](https://github.com/google/A2UI/blob/main/samples/mcp/a2ui-in-mcpapps/README.md) を参照してください。
...
5.  **Response**: MCP Server が tool を実行し、`application/json+a2ui` resource を含む result を返します。
...
2.  サンプルの [`inline.js`](https://github.com/google/A2UI/blob/main/samples/mcp/a2ui-in-mcpapps/server/apps/src/inline.js) のような post-build script を使い、`index.html` を読み取って、外部の `<script src="...">` と `<link rel="stylesheet" href="...">` タグを、実際のファイル内容を含む inline `<script>` と `<style>` タグへ置き換えます。
...
3.  Host から応答を受け取ったら、content 内の `application/json+a2ui` mimeType を探します。
4.  JSON text をパースし、A2UI [`MessageProcessor`](https://github.com/google/A2UI/blob/main/renderers/angular/src/v0_8/data/processor.ts) に渡します。
