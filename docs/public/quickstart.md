...
## ステップ 3: Lit クライアントのサンプルディレクトリへ移動する

クライアントアプリケーションのソースコードは `samples/client/lit/shell` にあります。デモを実行するために、その親にあたる samples ディレクトリへ移動します。
...
デモランチャーを実行します（Corepack を有効にしておくと、Node が正しいバージョンの Yarn を自動的に取得します）。
...
エージェントとクライアントを別々のターミナルで実行したい場合や、トラブルシューティングが必要な場合は、こちらの方法を使います。
...
- **[既存のエージェントアプリで使う](guides/a2ui-with-any-agent-framework.md)**: CopilotKit + AG-UI 経由で、ADK、LangGraph、CrewAI、Mastra、または独自サービスに A2UI を追加する
...
ブラウザを開いたときに `ERR_CONNECTION_REFUSED` が出ても心配いりません。これは既知の競合です ([#587](https://github.com/a2ui-project/a2ui/issues/587))。Web アプリが Python エージェントより先に起動するためです。数秒待ってからページを再読み込みしてください。
...
- [GitHub Issues](https://github.com/a2ui-project/a2ui/issues) を確認する
- [samples/client/lit/README.md](../../samples/client/lit) を読む
- コミュニティの議論に参加する
