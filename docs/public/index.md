...
| バージョン | ステータス | 説明 |
|---------|--------|-------------|
| **[v1.0](specification/v1.0-a2ui.md)** | **候補版** | リリース候補です。クライアントからサーバーへの RPC（`actionResponse`）とアクション ID を追加し、`theme` を `surfaceProperties` に名称変更しました（草案時点では v0.10 と呼ばれていました）。 [進化ガイド →](specification/v1.0-evolution-guide.md) |
| **[v0.9.1](specification/v0.9.1-a2ui.md)** | **現行版** | 現在の本番向けリリースです。v0.9 に対する軽微な改良として、`application/a2ui+json` MIME タイプを標準化し、`surfaceId` の制約を緩和しました。 [進化ガイド →](specification/v0.9.1-evolution-guide.md) |
| **[v0.9](specification/v0.9-a2ui.md)** | **安定版** | 一つ前の安定版です。Prompt-First への思想転換を行い、`createSurface`、クライアント側関数、カスタムカタログ、モジュール化されたスキーマ、バリデーション失敗時のエラー形式を導入しました。 [進化ガイド →](specification/v0.9-evolution-guide.md) |
| **[v0.8](specification/v0.8-a2ui.md)** | **レガシー** | レガシーバージョンです。Structured Output を優先していた頃の基盤で、基本的なサーフェス、コンポーネント、データバインディング、隣接リストモデルを含みます。 |

[CopilotKit](https://docs.copilotkit.ai/generative-ui/a2ui) とオープンソースコミュニティの貢献を受けながら、A2UI は Google によって提供されており、[GitHub](https://github.com/a2ui-project/a2ui) で活発に開発されています。
...
- [A2UI 仕様](specification/v0.9.1-a2ui.md)（v0.9.1 現行版、v1.0 候補版）
...
- :material-react:{ .lg .middle } **[任意のエージェントフレームワークとハーネスで A2UI を使う](guides/a2ui-with-any-agent-framework.md)**

    ---

    好みのエージェントフレームワーク向けに AG-UI アプリまたはハーネスをスキャフォールドし、クライアントのサーフェスで A2UI レンダリングを有効にします。

    [:octicons-arrow-right-24: 任意のエージェントで使う](guides/a2ui-with-any-agent-framework.md)

- :material-palette-outline:{ .lg .middle } **[A2UI Composer](https://a2ui-composer.ag-ui.com/)**

    ---

    ビジュアルエディターで A2UI の JSON を生成できます。インストール不要です。出力を任意のエージェントプロンプトに貼り付けるだけです。

    [:octicons-arrow-right-24: Composer を開く](https://a2ui-composer.ag-ui.com/)

- :material-play-circle-outline:{ .lg .middle } **[A2UI Theater](https://a2ui-composer.ag-ui.com/theater)**

    ---

    Lit、React、Angular の各レンダラーで、あらかじめ用意された A2UI ストリーミングシナリオをステップ実行できます。コードを書く前にプロトコルの動きを確認できます。

    [:octicons-arrow-right-24: プレイグラウンドを開く](https://a2ui-composer.ag-ui.com/theater)
...
- :material-file-document:{ .lg .middle } **プロトコル仕様**
    ...
    完全な技術仕様を確認できます。[v0.8（レガシー）](specification/v0.8-a2ui.md) · [v0.9（安定版）](specification/v0.9-a2ui.md) · [v0.9.1（現行版）](specification/v0.9.1-a2ui.md) · [v1.0（候補版）](specification/v1.0-a2ui.md)

    [:octicons-arrow-right-24: v0.9.1 仕様を読む](specification/v0.9.1-a2ui.md)
