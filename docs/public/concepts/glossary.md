...
システムプロンプトの指示が生成される前、あるいはペイロード検証スキーマがコンパイルされる前に、手つかずの **Catalog** をプログラムでフィルタリング・調整・変形するルールセットです。

#### Catalog Transformer が必要な理由

カタログはレンダラーがサポートするすべての UI コンポーネントと関数の完全な仕様を提供しますが、エージェントの実装では、特定のユースケースに合わせてカタログを刈り込んだり制限したりする必要が頻繁に生じます。

- **コンテキストウィンドウのトークン最適化**: カタログは数十のコンポーネントやロジック関数を定義できます。すべてのコンポーネントの JSON スキーマを LLM のシステムプロンプトに注入すると、コンテキストウィンドウのトークンを大量に消費し、プロンプトのレイテンシが増加し、LLM の推論コストも膨らみます。刈り込み (pruning) 用のトランスフォーマーは、システムプロンプトの指示を、そのエージェントのドメインタスクに関係するコンポーネントだけに限定します。
- **タスク固有の機能ガードレール**: 特定のエージェントワークフローやセキュリティロールでは、インタラクティブな機能を制限する必要がある場合があります (例: ゲストモードでは表示用のカードとテキストは許可しつつ、フォーム入力や管理用コンポーネントは無効化する)。
- **モデルシグネチャの削減**: より小さな、あるいはマイクロ LLM 向けに、関数ルールや検証項目を刈り込んでコンパクトなプロンプト指示ブロックを生成します。

#### Catalog Transformer の例

- `ComponentPruningTransformer`: 許可されたコンポーネント名のリストに基づいてカタログをフィルタリングするユーティリティクラスです。

- `FunctionPruningTransformer`: 許可された関数名のリストに基づいてカタログをフィルタリングするユーティリティクラスです。

### Capabilities Object (機能オブジェクト)

利用可能なカタログのセットです。このセットを通じて、エージェントまたはレンダラーは自身がサポートしているものを通知（advertise）し、相手側が何を送信できるかを把握できるようにします。

たとえば、レンダラーが Basic、Hotels、Flights の各カタログをサポートしている場合、それらを Capabilities Object 内で通知します。

Capabilities Object は、[A2UI メッセージ](#a2ui-message)としてではなく、トランスポートのメタデータまたは初期化ペイロードを通じて交換されます。正確な配置場所はトランスポートによって異なります。

このオブジェクトは、プロトコルの以前のバージョンでは異なる名前で呼ばれていました。v0.9 および v0.9.1 ではクライアントおよびサーバーのケーパビリティ (client and server capabilities) と呼ばれ、v0.8 ではクライアント側のスキーマのみが定義されていました。
...
LLM のテキスト出力の中で A2UI ペイロードのコードブロックの範囲を示すために使う囲み区切りタグです (`<a2ui-json>`、`<a2ui>` など)。

LLM は同じターンの中で会話用のプレーンテキストと UI ペイロードをまとめてストリーミングするため、A2UI Tag は構造化された UI ブロックを会話テキストから切り離す明示的な構文上の境界として機能します。

### Tag Unwrapping

レスポンス解析の最初のフェーズ (`unwrap`) です。パーサーが LLM のテキストレスポンスから開始・終了の **A2UI Tag** を探し、UI ではない会話テキスト (例: _「要約は次のとおりです:」_) と、タグで囲まれた生の UI コードブロックとを切り分けます。

### Compilation

レスポンス解析の 2 番目のフェーズ (`compile`) です。unwrapping で抽出されたモデル生成の UI コード文字列ブロック (標準 JSON、Express DSL 構文、Elemental HTML タグなど) を解析・逆コンパイルし、標準的な A2UI プロトコルのペイロード辞書 (`createSurface`、`updateDataModel`) へと変換します。
