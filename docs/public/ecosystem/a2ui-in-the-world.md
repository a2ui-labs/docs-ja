...
> **注:** Gemini Enterprise での A2UI 描画は、現在のところ Agent-to-Agent（A2A）パス（`a2aAgentDefinition`）で登録されたセルフサーブエージェントでのみ利用できます。Vertex AI Agent Engine にデプロイされ、`adkAgentDefinition` パスで登録されたマネージドエージェントは、現時点では A2UI 描画に対応していません。
...
- **Agent SDK**: [A2UI Python agent SDK](https://github.com/a2ui-project/a2ui/tree/main/agent_sdks/python) が、エージェントから A2UI を生成するための ADK 拡張を提供
...
[AG-UI](https://ag-ui.com/) はプロトコルを提供し、[CopilotKit](https://www.copilotkit.ai/) はエージェントアプリケーションを構築するための主要なフルスタックフレームワークを提供します。両者とも**導入初日から A2UI に対応**しています。
...
- **フレームワーク統合**: CopilotKit を通じて React、Vue、Angular などのアプリ環境と統合
...
>
> **— Atai Barkai**, CopilotKit および AG-UI の創業者
...
**本番で A2UI を使っていますか？** [GitHub Discussions](https://github.com/a2ui-project/a2ui/discussions) でぜひ体験を共有してください。
